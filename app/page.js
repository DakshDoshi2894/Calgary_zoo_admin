...
export default function Home() {
  ...
  // Added manual override functionality: if manualOverdue is true, then overdue; if manualOverride true then active
        ...
        items_html: items.map(item => `
          <tr>
            ...
            <td style="color: blue; text-decoration: underline;">
              <a href="${item.itemID ? item.itemID : '#'}" target="_blank"> View Item </a>
            </td>
  ...
  const handleMarkAsActive = async (itemId) => {
    try {
      await updateDoc(doc(db, "CheckedOutItems", itemId), { manualOverride: true, manualOverdue: false });
  ...
  const handleMarkAsOverdue = async (itemId) => {
    try {
      await updateDoc(doc(db, "CheckedOutItems", itemId), { manualOverdue: true, manualOverride: false });
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
                              ...
                              <tr key={item.id}>
                                ...
                                {(activeTab === 'checked-out' || activeTab === 'overdue') && (
                                  <td className="px-6 py-4 whitespace-nowrap space-x-2">
                                    <button onClick={() => handleCheckIn(item.id)} className="px-3 py-1 bg-blue-500 hover:bg-blue-600 text-white text-xs rounded">
                                      Check In
                                    </button>
                                    {activeTab === 'checked-out' && !isOverdue && (
                                      <button onClick={() => handleMarkAsOverdue(item.id)} className="px-3 py-1 bg-orange-500 hover:bg-orange-600 text-white text-xs rounded">
                                        Mark as Overdue
                                      </button>
                                    )}
                                    {activeTab === 'overdue' && isOverdue && (
                                      <button onClick={() => handleMarkAsActive(item.id)} className="px-3 py-1 bg-green-500 hover:bg-green-600 text-white text-xs rounded">
                                        Mark as Active
                                      </button>
                                    )}
                                  </td>
      ...
      <footer className="bg-white dark:bg-gray-800 shadow">
        <div className="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8">
          <p className="text-center text-sm text-gray-500 dark:text-gray-400">© 2025 Calgary Zoo</p>
        </div>
      </footer>
    </div>
  );
